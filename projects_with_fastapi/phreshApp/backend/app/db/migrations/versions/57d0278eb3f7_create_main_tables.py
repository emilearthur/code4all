"""create_main_tables
Revision ID: 57d0278eb3f7
Revises:
Create Date: 2021-03-16 20:41:27.750027
"""
...
def create_offers_table() -> None:
    op.create_table(
        ...
        sa.Column("user_id", sa.Integer, sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True,),  # "user" is a reserved word for postgres
        sa.Column("cleaning_id", sa.Integer, sa.ForeignKey("cleanings.id", ondelete="CASCADE"), nullable=False, index=True,),
        sa.Column("status", sa.Text, nullable=False, server_default="pending", index=True),
        *timestamps(),
    )
    # this ensures only unqiue combinations are allowed. So user can offer their services for a particlar cleaning job once
...
def create_cleaner_evaluations_table() -> None:
    """
    Owner of a cleaning job should be able to evalute a cleaner's execution of the job.
    ...
    - Rating split into sections
        ...
        - completeness: how thorugh were they? did everything get cleaned as it should have been?
        - efficiency: how quickly and effectively did they get the job done?
        - overall: what's the consenus rating for this cleaning job?
    """
    op.create_table(
        "cleaning_to_cleaner_evaluations",
        sa.Column("cleaning_id", sa.Integer, sa.ForeignKey("cleanings.id", ondelete="SET NULL"), nullable=False, index=True,),  # job that was completed
        sa.Column("cleaner_id", sa.Integer, sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=False, index=True,),   # user who completed the job
    ...
    op.create_primary_key("pk_cleaning_to_cleaner_evaluations", "cleaning_to_cleaner_evaluations", ["cleaning_id", "cleaner_id"])
    op.execute(
        """
