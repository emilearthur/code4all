...
from app.api.dependencies.offers import (check_offer_create_permissions, check_offer_list_permissions, check_offer_get_permissions,
                                         get_offer_for_cleaning_from_user_by_path, list_offers_for_cleaning_by_id_from_path,
                                         check_offer_acceptance_permissions, check_offer_cancel_permissions,
                                         get_offer_for_cleaning_from_current_user, check_offer_rescind_permissions)
...
async def create_offer(cleaning: CleaningInDB = Depends(get_cleaning_by_id_from_path),
                       current_user: UserInDB = Depends(get_current_active_user),
                       offers_repo: OffersRepository = Depends(get_repository(OffersRepository)), ) -> OfferPublic:
    return await offers_repo.create_offer_for_cleaning(new_offer=OfferCreate(cleaning_id=cleaning.id, user_id=current_user.id))
...
async def list_offers_for_cleaning(offers: List[OfferInDB] = Depends(list_offers_for_cleaning_by_id_from_path)) -> List[OfferPublic]:
    return offers
...
async def accept_offer_from_user(offer: OfferInDB = Depends(get_offer_for_cleaning_from_user_by_path),
                                 offers_repo: OffersRepository = Depends(get_repository(OffersRepository)),) -> OfferPublic:
...
async def cancel_offer_from_user(offer: OfferInDB = Depends(get_offer_for_cleaning_from_current_user),
                                 offers_repo: OffersRepository = Depends(get_repository(OffersRepository)),) -> OfferPublic:
...
async def rescind_offer_fron_user(offer: OfferInDB = Depends(get_offer_for_cleaning_from_current_user),
                                  offers_repo: OffersRepository = Depends(get_repository(OffersRepository)),) -> OfferPublic:
