...
class TestUserRegistration:
    async def test_users_can_register_successfully(self, app: FastAPI,
                                                   client: AsyncClient,
                                                   db: Database,) -> None:
        user_repo = UsersRepository(db)
        new_user = {"email": "emilexarthur@gmail.com", "username": "emilexarthur", "password": "emilexarthur"}
        ...
        # ensure that the user new exist in the db
        user_in_db = await user_repo.get_user_by_email(email=new_user["email"], populate=False)
        assert user_in_db is not None
        assert user_in_db.email == new_user["email"]
        assert user_in_db.username == new_user["username"]

        # check that the user returned in the response is equal to the user in the db
    ...
    @pytest.mark.parametrize(
        ...
        (
            ("email", "emilexarthur@gmail.com", 400),
            ("username", "emilexarthur", 400),
    ...
    async def test_user_registration_fails_when_credentials_are_taken(self, app: FastAPI,
                                                                      client: AsyncClient,
                                                                      db: Database,
                                                                      attr: str,
                                                                      value: str,
    ...
    async def test_users_saved_password_is_hashed_and_has_salt(self, app: FastAPI,
                                                               client: AsyncClient,
                                                               db: Database,) -> None:
        user_repo = UsersRepository(db)
        new_user = {"email": "mum@gmail.com", "username": "mumak", "password": "somethingmain"}
        # send post request to create user
        res = await client.post(app.url_path_for("users:register-new-user"), json={"new_user": new_user})
        assert res.status_code == status.HTTP_201_CREATED
        # ensuring pwd is hashed in db
        user_in_db = await user_repo.get_user_by_email(email=new_user["email"], populate=False)
        assert user_in_db is not None
        assert user_in_db.salt is not None and user_in_db.salt != "123"
        assert auth_service.verify_password(plain_password=new_user["password"],
                                            salt=user_in_db.salt,
                                            hashed_password=user_in_db.password,)


class TestAuthTokens:
    async def test_can_create_access_token_sucessfully(self, app: FastAPI,
                                                       client: AsyncClient,
    ...
    async def test_token_missing_user_is_invalid(self, app: FastAPI, client: AsyncClient) -> None:
        access_token = auth_service.create_access_token_for_user(user=None,
                                                                 secret_key=str(SECRET_KEY),
                                                                 audience=JWT_AUDIENCE,
                                                                 expires_in=ACCESS_TOKEN_EXPIRE_MINUTES,
                                                                 )
    ...
    async def test_invalid_token_content_raises_error(self, app: FastAPI,
                                                      client: AsyncClient,
                                                      test_user: UserInDB,
    ...
    async def test_can_retrive_username_from_token(self, app: FastAPI,
                                                   client: AsyncClient,
                                                   test_user: UserInDB) -> None:
    ...
    async def test_error_when_token_or_secret_is_wrong(self, app: FastAPI,
                                                       client: AsyncClient,
                                                       test_user: UserInDB,
...
class TestUserRegisteration:
    async def test_users_can_register_successfully(self, app: FastAPI,
                                                   ...
                                                   db: Database) -> None:
        user_repo = UsersRepository(db)
        new_user = {"email": "frankmayz@gmail.com", "username": "frankmayz", "password": "somethingmore"}
        # check if user does not exist
        user_in_db = await user_repo.get_user_by_email(email=new_user["email"])
        assert user_in_db is None
        # send post reqest to create user
        res = await client.post(app.url_path_for("users:register-new-user"), json={"new_user": new_user})
        assert res.status_code == status.HTTP_201_CREATED

        # ensure user exist in db
        user_in_db = await user_repo.get_user_by_email(email=new_user["email"], populate=False)
        assert user_in_db is not None
        assert user_in_db.email == new_user["email"]
        assert user_in_db.username == new_user["username"]

        # check user returned in response is same as user in db
        created_user = UserPublic(**res.json()).dict(exclude={"access_token", "profile"})
        assert created_user == user_in_db.dict(exclude={"password", "salt"})


class TestUserLogin:
    async def test_user_can_login_successfully_and_receives_valid_token(self, app: FastAPI,
                                                                        client: AsyncClient,
                                                                        test_user: UserCreate,) -> None:
        ...
        # check if token exists in response and has user encoded within it
    ...
    async def test_user_with_wrong_creds_doesnt_receive_token(self, app: FastAPI,
                                                              client: AsyncClient,
                                                              test_user: UserInDB,
                                                              credential: str,
                                                              wrong_value: int,
        ...
        user_data["password"] = "hititupandgo"
...
class TestUserMe:
    async def test_autheniticated_user_can_retrieve_own_data(self, app: FastAPI,
    ...
    async def tes_user_cannot_access_own_data_if_not_authenticated(self,
                                                                   app: FastAPI,
                                                                   client: AsyncClient,
                                                                   test_user: UserInDB,) -> None:
    ...
    async def test_user_cannot_access_own_data_with_incorrect_jwt_prefix(self, app: FastAPI,
                                                                         client: AsyncClient,
                                                                         test_user: UserInDB,
                                                                         jwt_prefix: str,) -> None:
        token = auth_service.create_access_token_for_user(user=test_user, secret_key=str(SECRET_KEY))
