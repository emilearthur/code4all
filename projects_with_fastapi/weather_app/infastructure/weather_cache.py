...
def get_weather(city: str, state: Optional[str], country: str,
...
def __create_key(city: str, state: str, country: str,
                 units: str) -> Tuple[str, str, str, str]:
    if not city or not country or not units:
        raise Exception("City, country, and unit are required")

    if not state:
        state = ""

    return city.strip().lower(), state.strip().lower(), units.strip().lower()
